...
cube1 = iris.load('japp_default.nc')[0][:46,:,:]
cube2 = iris.load('japp_lehtinen.nc')[0][:46,:,:]
"""
...
# plots of Jveh
################################################33333
cube3 = iris.load('jveh_default.nc')[0][:40,:,:]
cube4 = iris.load('jveh_lehtinen.nc')[0][:40,:,:]
plt.figure()
plt.xlabel('Jveh')
plt.ylabel('Model_level_number')
level_num_all = np.arange(1,41,1)
...
cube5 = iris.load('condsink_default.nc')[0]
plt.figure()
...
cube6 = iris.load('coag3nm_lehtinen.nc')[0]
cube7 = iris.load('coag1nm_lehtinen.nc')[0]
plt.figure()
...
plt.legend(('3nm coagsink','1nm coagsink'))
plt.grid(True)


# plot of coefficient/power m

cube8 = iris.load('m_leh_lehtinen.nc')[0]

plt.figure()
plt.xlabel('m (factor)')
plt.ylabel('Model_level_number')
level_num_all = np.arange(1,86,1)
plt.plot((cube8[:,72,96].data),level_num_all,'b*-')
