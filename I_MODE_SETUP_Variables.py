...
if I_MODE_SETUP==8:
  ...
  #  sigma=[1.59,1.59,1.40,2.0,1.59,1.59,2.0]
    ...
    ddplim0=[1.0e-9,1.0e-8,1.0e-7,0.5e-6,1.0e-8]
  #  ddplim0=[1.0e-9,1.0e-8,1.0e-7,0.5e-6,1.0e-8,1.0e-7,1.0e-6]
    # Upper Limit of Mode: ddplim1
    ddplim1=[1.0e-8,1.0e-7,0.5e-6,1.0e-5,1.0e-7]
  #  ddplim1=[1.0e-8,1.0e-7,0.5e-6,1.0e-5,1.0e-7,1.0e-6,1.0e-5]
    ...
    # Component names
    component_names= ["so4","bc","oc","ss"]#ANANTH CHANGE
  #  component_names= ["so4","bc","oc","ss","dust"]#,"sec_org"]#do we have secondary organics??
    #kappa_component=[   0.61,    0.00,    0.200,    1.280]#ANANTH CHANGED KAPPA OF THIRD COMPONENT

    #kappa_component=[   0.61,    0.00,    0.20,    1.280]#ANANTH CHANGED KAPPA TO ZERO BECAUSE IN DEFAULT MET OFFICE MODEL THE VALUE IS ZERO
   
    kappa_component=[   0.61,    0.00,    0.00,    1.280] # MET OFFICE DEFAULT VALUE IS ZERO
  #  kappa_component=[   0.61,    0.00,    0.10,    1.28,    0.00]#, 0.1]#check seconday organics I setted it to the same as organics but need to check. Jesus
    # Molar Mass kg / mol
    mm= [0.098,0.012,0.0168,0.05844]#,0.0168] #ANANTH
  #  mm= [0.098,0.012,0.0168,0.05844,0.100]#,0.0168]
    # Density
    rhocomp=[1769.0,1500.0,1500.0,1600.0]#,1500.0]#kg/m3
