...
"""
import sys
dir_scripts='/group_workspaces/jasmin2/gassp/eeara'
...
directory='/group_workspaces/jasmin2/gassp/eeara/'
#folder= directory+'umnsa_glm_nc_time/'
folder=directory +'All_time_steps/' #+'All_months'
...
print particle_density_of_air.data[0,0,0]
...
for name in vd.variable_reference_name.keys():
    if 'nbr_' in name:
        ...
        print particles_concentration['n'+name[3:]].data[0,0,0]
...
for name in mass_mixing_ratio.keys():
    ...
    del mass_mixing_ratio[name]

if PM==1:
    mass_concentration_per_mode={}
    for mode_name in ims.mode_names[0:5]:
        print mode_name
    ...
    for comp_name in ims.component_names[0:4]:
        cubes_to_add=[mass_concentration[key] for key in mass_concentration.keys() if comp_name in key]
        mass_concentration_per_component['mcon_'+comp_name]=np.sum(cubes_to_add)
        mass_concentration_per_component['mcon_'+comp_name].long_name='total_mass_concentration_of_%s'%comp_name
        mass_concentration_per_component['mcon_'+comp_name]._var_name='mcon_'+comp_name
        save_cube(mass_concentration_per_component['mcon_'+comp_name])
...
for mode_name in ims.mode_names[0:5]:
    ...
    #print keys
    for comp_name in ims.component_names[0:4]:
        print ims.species_attributes[comp_name].kappa
        ...
        if any(keys_comp):
            ...
            if PM==0:
                del mass_concentration[keys_comp[0]]
...
for mode_name in ims.mode_names[0:5]:
    ...
    mode_radius['rad_'+mode_name]=0.5*iris.analysis.maths.exponentiate((6*mode_volume['vol_'+mode_name]/particles_concentration['n_'+mode_name])/(np.pi*np.exp(4.5*(np.log(ims.modal_attributes[mode_name].sigma))**2.0)),1./3.)
        ...
        if ims.modal_attributes[mode_name].modesol:
            for comp_name in ims.component_names[0:4]:
                if 'vol_'+comp_name+'_'+mode_name in mode_volume_per_component.keys():
                    ...
                    del mode_volume_per_component['vol_'+comp_name+'_'+mode_name]
        kappa_weighted_per_mode[mode_name]=np.sum(cubes_to_add)
#       kappa_weighted_per_mode[mode_name]._var_name='kappa'+mode_name
#       save_cube(kappa_weighted_per_mode[mode_name])
    for supersaturation in [0.2,0.4]:
        ...
        for mode_name in ims.mode_names[0:5]:
            if ims.modal_attributes[mode_name].modesol:
                critical_diameter_per_mode[mode_name]=(4.*A_kohler**3/(27.*kappa_weighted_per_mode[mode_name].data*(np.log(1+supersaturation/100.))**2.0))**(1./3.)
                N=particles_concentration['n_'+mode_name]
                rbar=mode_radius['rad_'+mode_name].data
                sigma=ims.modal_attributes[mode_name].sigma
                Rcritcube=mode_radius['rad_'+mode_name].copy(critical_diameter_per_mode[mode_name]/2.)
                Rcrit=critical_diameter_per_mode[mode_name]/2.
                Rcritcube.long_name='rcrit_'+mode_name
                Rcritcube._var_name='rcrit_'+mode_name
                save_cube(Rcritcube)
